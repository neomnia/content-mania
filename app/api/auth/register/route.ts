...
export async function POST(request: NextRequest) {
  try {
    ...
    let ownerRole
    try {
      ownerRole = await db.query.roles.findFirst({
        where: eq(roles.name, "owner"),
      })

      // If owner role doesn't exist, create it
      if (!ownerRole) {
        console.log("[v0] Owner role not found, creating it")
        ;[ownerRole] = await db
          .insert(roles)
          .values({
            name: "owner",
          ...
          .returning()
        console.log("[v0] Owner role created:", ownerRole.id)
      } else {
        console.log("[v0] Owner role found:", ownerRole.id)
      }
    } catch (dbError) {
      console.error("[v0] Error with owner role:", dbError)
    ...
    if (ownerRole) {
      try {
        ...
        console.log("[v0] Owner role assigned to user")
      } catch (dbError) {
