          ...
          >
            ...
            <div data-slot="card-header" className="@container/card-header grid auto-rows-min grid-rows-[auto_auto] items-start gap-2 px-6 has-data-[slot=card-action]:grid-cols-[1fr_auto] [.border-b]:pb-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  {Icon && (
                    <div className="h-12 w-12 rounded-full bg-[#CD7F32]/20 flex items-center justify-center">
                      <Icon className="h-6 w-6 text-[#CD7F32]" />
                    </div>
                  )}
                  <div>
                ...
                {isFree && (
                  <span data-slot="badge" className="inline-flex items-center justify-center rounded-md border px-2 py-0.5 text-xs font-medium w-fit whitespace-nowrap shrink-0 bg-[#CD7F32] text-white">
                    FREE
                  </span>
                )}
              </div>

              {/* Priorité : Taux horaire > Prix standard > Gratuit */}
              {hasHourlyRate ? (
                <div className="mt-6 flex items-baseline gap-1">
                  <span className="text-4xl font-bold">
                    {product.currency === 'EUR' ? '€' : '$'}
                    {product.hourlyRate / 100}
                  </span>
            ...
            {isFree && (
              <div data-slot="card-footer" className="flex items-center px-6 [.border-t]:pt-6">
                <button 
                  onClick={() => handlePurchase(product)}
                  data-slot="button" 
                  className="inline-flex items-center justify-center gap-2 whitespace-nowrap text-sm font-medium transition-all h-10 rounded-md px-6 w-full bg-[#CD7F32] hover:bg-[#B26B27] text-primary-foreground"
                >
