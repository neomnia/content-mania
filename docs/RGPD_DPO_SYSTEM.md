...
1. **`db/push-schema.ts`** (ligne 314-320)
```typescript
INSERT INTO users (
...
1. ✅ `db/push-schema.ts` - Ajout `is_dpo` et `is_site_manager` lors de la création du super admin
2. ✅ `scripts/seed-database.ts` - Ajout `isDpo` et `isSiteManager` lors du seed
