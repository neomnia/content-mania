...
1. **Modification du Schéma** : Éditer `db/schema.ts`.
2. **Génération de Migration** : `npm run db:generate`
3. **Application (Dev/Preview)** : `npm run db:push`
4. **Déploiement Production** : Le script de build gère automatiquement les migrations si configuré.
